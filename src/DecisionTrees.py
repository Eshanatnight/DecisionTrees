"""
A Decision Tree Binary Classifier.
...
class DecisionTree:
    ...
    def __init__(self, max_depth: int = 6, depth: int = 1):
    ...
    def fit(self, data: pd.DataFrame, target: str):
        ...
        self.independent: list = self.data.columns.tolist()
        self.independent.remove(target)

        if self.depth <= self.max_depth:
            self.__validate_data()
            self.impurity_score: float = self.__calculate_impurity_score(self.data[self.target])
    ...
    def __create_branches(self):
        ...
        create the branches
        """
        self.left: DecisionTree = DecisionTree(max_depth=self.max_depth, depth=self.depth + 1)
        self.right: DecisionTree = DecisionTree(max_depth=self.max_depth, depth=self.depth + 1)
        left_rows: pd.DataFrame = self.data[self.data[self.split_feature] <= self.criteria]
        right_rows: pd.DataFrame = self.data[self.data[self.split_feature] > self.criteria]

        self.left.fit(data = left_rows, target = self.target)
        self.right.fit(data = right_rows, target = self.target)


    def __calculate_impurity_score(self, data) -> float:
        """
        get the impurity score
        """
    ...
    def __find_best_split_for_column(self, col):
        ...
        for val in unique_values:
            ...
            left_impurity: float = self.__calculate_impurity_score(left_data[self.target])
            right_impurity: float = self.__calculate_impurity_score(right_data[self.target])
            score: float = self.__calculate_information_gain(left_count = len(left_data),
                    left_impurity = left_impurity, right_count = len(right_data),
    ...
    def __calculate_information_gain(self, left_count, left_impurity, right_count, right_impurity) -> float:
        """
        get the imformation gain metric
        """
